...
const calculateCappedHeights_1 = require("../calculateCappedHeights");
const open = (scrollHeight) => ({
    scrollHeight,
    isOpen: true,
});
const closed = (scrollHeight) => ({
    scrollHeight,
    isOpen: false,
});
describe("calculateCappedHeights", () => {
    ...
    it("returns zeros when freeSpace is 0", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(0, [open(100), open(100)], 0);
        expect(r.heights).toEqual([0, 0]);
        expect(r.capped).toEqual([false, false]);
    });
    it("returns zeros when no lists are provided", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [], 0);
    ...
    it("gives each list its natural size when total fits", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(100), open(100)], 0);
        expect(r.heights).toEqual([100, 100]);
        expect(r.capped).toEqual([false, false]);
    });
    it("gives each list its natural size when total exactly equals free", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(200), open(200)], 1);
    ...
    it("caps the active list when it alone causes overflow", () => {
        ...
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(500), open(100)], 0);
    ...
    it("caps the active list (index 1) when it causes overflow", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(100), open(500)], 1);
    ...
    it("caps both lists at equal share when both exceed it", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(500), open(600)], 0);
        expect(r.heights).toEqual([200, 200]);
        expect(r.capped).toEqual([true, true]);
    });
    it("caps both lists at equal share regardless of active index", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(500), open(600)], 1);
        expect(r.heights).toEqual([200, 200]);
        expect(r.capped).toEqual([true, true]);
    });
    ...
    it("redistributes surplus when active is smaller than equal share", () => {
        ...
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(100), open(500)], 0);
        expect(r.heights).toEqual([100, 300]);
        expect(r.capped).toEqual([false, true]);
    });
    // ── Moderate overflow ───────────────────────────────────────────
    it("caps active and keeps inactive uncapped when inactive fits in equal share", () => {
        ...
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(250), open(180)], 0);
        expect(r.heights).toEqual([220, 180]);
        expect(r.capped).toEqual([true, false]);
    });
    // ── Closed panels ──────────────────────────────────────────────
    it("gives all space to the open list when one is closed", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [open(500), closed(300)], 0);
    ...
    it("handles only inactive list open", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [closed(500), open(200)], 0);
    ...
    it("handles both lists closed", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(400, [closed(500), closed(300)], 0);
        expect(r.heights).toEqual([0, 0]);
        expect(r.capped).toEqual([false, false]);
    });
    // ── Odd free space (rounding) ──────────────────────────────────
    it("handles odd freeSpace without losing a pixel", () => {
        const r = (0, calculateCappedHeights_1.calculateCappedHeights)(401, [open(500), open(500)], 0);
        // equalShare = 200; inactive gets 200, active gets 201
        expect(r.heights[0] + r.heights[1]).toBe(401);
        expect(r.capped).toEqual([true, true]);
    });
});
//# sourceMappingURL=calculateCappedHeights.test.js.map